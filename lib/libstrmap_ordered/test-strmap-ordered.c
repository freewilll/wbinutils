...
int main() {
    StrMapOrdered *map = new_strmap_ordered();
    ...
    for (int i = 0; i < COUNT; i++) {
        ...
        strmap_ordered_put(map, key, value);
    }
    ...
    for (int i = 0; i < COUNT; i++) {
        ...
        sprintf(key, "foo %d", i);
        sprintf(value, "bar %d", i);
        char *got_value = strmap_ordered_get(map, key);
        if (!got_value) panic("Didn't get a match\n");
        if (strcmp(value, got_value)) panic("Got something horrible 1");

        got_value = strmap_ordered_get(map, "nuttin");
        if (got_value) panic("Expected no match");
    }

    // Test iteration
    int i = 0;
    for (StrMapOrderedIterator it = strmap_ordered_iterator(map); !strmap_ordered_iterator_finished(&it); strmap_ordered_iterator_next(&it), i++) {
        const char *value = strmap_ordered_iterator_key(&it);
        int int_value = atoi(&(value[4]));
        if (int_value != i) panic("Iteration returned wrong value");
    }
    ...
    for (StrMapOrderedIterator it = strmap_ordered_iterator(map); !strmap_ordered_iterator_finished(&it); strmap_ordered_iterator_next(&it), i++) {
        const char *value = strmap_ordered_iterator_key(&it);
        int int_value = atoi(&(value[4]));
        if (int_value != i * 2) panic("Iteration returned wrong value");
    }

    // Insert/deletion thrashing in a new map. This tests rehashing without
    // a resize.
    map = new_strmap_ordered();
    for (int i = 0; i < COUNT; i++) {
        char *key = malloc(16);
        char value[] = "foo";
        sprintf(key, "foo %d", i);
        ...
        strmap_ordered_delete(map, key);
        got_value = strmap_ordered_get(map, key);
